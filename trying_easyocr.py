...
pipeline_bar.markdown(
    ...
    \n2. Написание функций обработки изображения: функции преобразования загруженного изображения в типа данных, доступный для приёма моделью, функции риования границ текста
    \n3. Загрузка изображения с текстом: с использованием библиотеки streamlit
    \n4. Выбор языка для определения: язык определяется пользователем
    \n5. Обработка изображения: библиотека EasyOCR может быть использована для распознавания текста с изображений более чем на 80 языках. Она использует CRAFT text detector обученный на 24 тыс. изображений текста. Принимает на вход трёхмерный массив чисел (numpy.ndarray), созданый на основе загруженного изображения теста/надписи​
    \n6. Проверка результата и корректировки:если результат неудовлетворительный, проводится корректировка гиперпараметров и функций из п.2
